...
if($_SERVER["REQUEST_METHOD"] == "POST" && strlen($_POST["submit"]) > 0)
{
	if(check_bitrix_sessid())
	{
		if(empty($arParams["REQUIRED_FIELDS"]) || !in_array("NONE", $arParams["REQUIRED_FIELDS"]))
		{
			...
			if((empty($arParams["REQUIRED_FIELDS"]) || in_array("TEL", $arParams["REQUIRED_FIELDS"])) && strlen($_POST["tel"]) <= 1)
				$arResult["ERROR_MESSAGE"][] = GetMessage("MF_REQ_TEL");
			if((empty($arParams["REQUIRED_FIELDS"]) || in_array("TOPIC", $arParams["REQUIRED_FIELDS"])) && strlen($_POST["topic"]) <= 2)
				$arResult["ERROR_MESSAGE"][] = GetMessage("MF_REQ_TOPIC");
			if((empty($arParams["REQUIRED_FIELDS"]) || in_array("TO", $arParams["REQUIRED_FIELDS"])) && !strlen($_POST["to"]) )
				$arResult["ERROR_MESSAGE"][] = GetMessage("MF_REQ_TO");
		}
		if(strlen($_POST["user_email"]) > 1 && !check_email($_POST["user_email"]))
			$arResult["ERROR_MESSAGE"][] = GetMessage("MF_EMAIL_NOT_VALID");
		if(strpos($_POST["tel"], '_')!==false)
			$arResult["ERROR_MESSAGE"][] = GetMessage("MF_TEL_NOT_VALID");
		...
		$arResult["NEW_EXT_FIELDS"]["TEL"] = htmlspecialcharsEx($_POST["tel"]);
		$arResult["NEW_EXT_FIELDS"]["TO"] = htmlspecialcharsEx($_POST["to"]);
		$arResult["NEW_EXT_FIELDS"]["TOPIC"] = htmlspecialcharsEx($_POST["topic"]);
		foreach($_POST["custom"] as $i => $custom_field)
		{
...
if(empty($arResult["ERROR_MESSAGE"]))
{
	if($USER->IsAuthorized())
	{
		...
		$arUser = $rsUser->Fetch();
		$arResult["NEW_EXT_FIELDS"]["TEL"] = htmlspecialcharsEx($arUser[PERSONAL_PHONE]?$arUser[PERSONAL_PHONE]:$arUser[PERSONAL_MOBILE]);
